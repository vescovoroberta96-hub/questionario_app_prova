...
st.set_page_config(
    page_title="Questionario di Rilevazione Biocidi e PMC",
    layout="centered",
)
...
OPZIONI_CANALI_DISTRIBUZIONE = ["Ospedaliero/sanitario", "Farmacia", "GDO"]
...
# ==============================================================================
# INIZIALIZZAZIONE CONNESSIONE E SESSION STATE
# ==============================================================================
try:
...
if "sostanze_list" not in st.session_state:
    st.session_state.sostanze_list = [get_empty_sostanza()]
...
# INTERFACCIA UTENTE
# ==============================================================================
st.title("Questionario di Rilevazione Biocidi e PMC")
st.markdown("Compilare tutti i campi richiesti per il censimento dei dati.")
...
ambiti_stab = st.multiselect(
    ...
    options=OPZIONI_AMBITI,
    help="È possibile selezionare sia Italia che Europa",
    key="ambiti_stab"
)
...
# 4. OPERATORE E SITO PRODUTTIVO
st.header("4. Operatore e Sito Produttivo")
col_op1, col_op2 = st.columns(2)

with col_op1:
    azienda = st.text_input("Azienda *", placeholder="Nome azienda")
    tipo_ubicazione = st.radio(
        "Ubicazione del Sito Produttivo *",
        ["In Europa", "All'estero"],
        horizontal=True
    )
    if tipo_ubicazione == "In Europa":
        dove_ubicazione = st.text_input("Dove (Paese europeo) *", placeholder="Es. Italia, Germania...")
    else:
        dove_ubicazione = st.text_input("Dove (Paese extra-UE) *", placeholder="Es. USA, Cina...")

    canali_distribuzione = st.multiselect(
        "Canali di distribuzione *",
        options=OPZIONI_CANALI_DISTRIBUZIONE
    )

with col_op2:
    cap_prod_attuale = st.text_input("Capacità produttiva attuale (L/kg al mese) *", placeholder="Es. 5000")
    cap_prod_emergenza = st.text_input("Capacità produttiva massima in emergenza (L/kg al mese) *", placeholder="Es. 15000")
    autonomia_scorte = st.text_input("Autonomia delle scorte (giorni) *", placeholder="Es. 30")

st.divider()

# 5. SCHEDA SOSTANZA ATTIVA
st.header("5. Schede Sostanze Attive")
st.caption("Compilare i dettagli per le sostanze attive presenti.")
...
for idx, sostanza in enumerate(st.session_state.sostanze_list):
    ...
    with st.expander(titolo_expander, expanded=(idx == 0)):
        ...
        if len(st.session_state.sostanze_list) > 1:
            st.button(f"Rimuovi Sostanza #{idx + 1}", on_click=rimuovi_sostanza, args=(idx,), key=f"btn_del_{idx}")

st.button("Aggiungi un'ulteriore sostanza attiva", on_click=aggiungi_sostanza, type="secondary")

st.divider()

# 6. SCHEDA PRODOTTO E STABILIMENTO
st.header("6. Scheda Prodotto e Stabilimento")
...
if st.button("Invia Questionario", type="primary", use_container_width=True):
    ...
    # Validazione Sezione 4 (Operatore)
    if not azienda.strip():
        errori.append("Inserire il nome dell'Azienda.")
    if not dove_ubicazione.strip():
        errori.append("Specificare la localizzazione del sito produttivo.")
    if not cap_prod_attuale.strip():
        errori.append("Inserire la capacità produttiva attuale.")
    if not cap_prod_emergenza.strip():
        errori.append("Inserire la capacità produttiva massima in emergenza.")
    if not autonomia_scorte.strip():
        errori.append("Inserire l'autonomia delle scorte in giorni.")
    if not canali_distribuzione:
        errori.append("Selezionare almeno un canale di distribuzione.")

    # Validazione Sezioni 5 e 6
    ...
    if not prodotti_validi:
        errori.append("Inserire il Nome di almeno un Prodotto nella sezione 6.")

    if errori:
        for err in errori:
            st.error(f"Attenzione: {err}")
    else:
        # 1. Costruzione DataFrame Operatore
        dati_operatore = [{
            "Azienda": azienda,
            "Ubicazione Sito": tipo_ubicazione,
            "Paese Sito Produttivo": dove_ubicazione,
            "Capacità Produttiva Attuale (L/kg mese)": cap_prod_attuale,
            "Capacità Max Emergenza (L/kg mese)": cap_prod_emergenza,
            "Autonomia Scorte (giorni)": autonomia_scorte,
            "Canali di Distribuzione": ", ".join(canali_distribuzione)
        }]

        # 2. Costruzione DataFrame Sostanze
        dati_sostanze = []
        ...
        # 3. Costruzione DataFrame Prodotti
        dati_prodotti = []
        ...
        df_op_nuovo = pd.DataFrame(dati_operatore)
        df_sos_nuove = pd.DataFrame(dati_sostanze)
        df_prod_nuovi = pd.DataFrame(dati_prodotti)

        # Invio a Google Sheets se la connessione è configurata
        if conn is not None:
            try:
                with st.spinner("Salvataggio in corso su Google Sheets..."):
                    df_op_old = conn.read(worksheet="Operatori", ttl=0)
                    df_sos_old = conn.read(worksheet="Sostanze Attive", ttl=0)
                    df_prod_old = conn.read(worksheet="Prodotti", ttl=0)

                    df_op_tot = pd.concat([df_op_old, df_op_nuovo], ignore_index=True)
                    df_sos_tot = pd.concat([df_sos_old, df_sos_nuove], ignore_index=True)
                    df_prod_tot = pd.concat([df_prod_old, df_prod_nuovi], ignore_index=True)

                    conn.update(worksheet="Operatori", data=df_op_tot)
                    conn.update(worksheet="Sostanze Attive", data=df_sos_tot)
                    conn.update(worksheet="Prodotti", data=df_prod_tot)

                st.success("Questionario inviato e registrato con successo su Google Sheets!")
            except Exception as e:
                st.warning(f"Questionario inviato ma si è verificato un errore nel salvataggio su Google Sheets: {e}")
        else:
            st.success("Questionario compilato correttamente!")

        # Visualizzazione Tabellare in pagina
        st.subheader("Tabella Operatore e Sito Produttivo")
        st.dataframe(df_op_nuovo, use_container_width=True)

        st.subheader("Tabella Sostanze Attive")
        st.dataframe(df_sos_nuove, use_container_width=True)

        st.subheader("Tabella Prodotti")
        st.dataframe(df_prod_nuovi, use_container_width=True)

        # Esportazione Excel Locale
        buffer_excel = io.BytesIO()
        with pd.ExcelWriter(buffer_excel, engine="openpyxl") as writer:
            df_op_nuovo.to_excel(writer, index=False, sheet_name="Operatore")
            df_sos_nuove.to_excel(writer, index=False, sheet_name="Sostanze Attive")
            df_prod_nuovi.to_excel(writer, index=False, sheet_name="Prodotti")

        st.download_button(
            "Scarica file Excel della sessione (.xlsx)",
            data=buffer_excel.getvalue(),
            file_name=f"risposte_{azienda.replace(' ', '_')}.xlsx",
