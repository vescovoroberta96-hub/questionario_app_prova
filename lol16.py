...
st.set_page_config(
    page_title="Questionario Biocidi e PMC",
    layout="centered",
)
...
# INTERFACCIA UTENTE
# ==============================================================================
st.title("Questionario di Rilevazione")
...
with col_op1:
    ...
    canali_distribuzione = st.multiselect(
        ...
        options=["Ospedaliero/sanitario", "Farmacia", "GDO"]
    )

with col_op2:
    cap_prod_attuale = st.text_input("Capacità produttiva attuale (L/kg al mese) *", placeholder="Es. 5.000")
    cap_prod_emergenza = st.text_input("Capacità produttiva massima in emergenza (L/kg al mese) *", placeholder="Es. 15.000")
    autonomia_scorte = st.text_input("Autonomia delle scorte (giorni) *", placeholder="Es. 30")

st.divider()

# 5. SCHEDA SOSTANZA ATTIVA
st.header("5. Schedes Sostanze Attive")
st.caption("Compila i dettagli per le sostanze attive presenti.")
...
if st.button("Invia Questionario", type="primary", use_container_width=True):
    ...
    if errori:
        ...
        # Invio a Google Sheets se la connessione è attiva
        if conn is not None:
            try:
                ...
                st.warning(f"Questionario inviato ma non è stato possibile salvare su Google Sheets: {e}")
        else:
